...
#                                 Inbox objects                                #
# ============================================================================ #
def getInboxHandle(clientID, h_objs, xBound, yBound):
    pos = arm.getObjPosition(h_objs)
    pos = npa(pos)

    # remove
    xf = np.logical_and(pos[:, 0] > xBound[0], pos[:, 0] < xBound[1])
    yf = np.logical_and(pos[:, 1] > yBound[0], pos[:, 1] < yBound[1])
    iInBox = np.logical_and(xf, yf)
    h_objs = h_objs[iInBox]
...
for iRun in range(nRun):
    ...
    # create obj
    h_objs = npa(arm.createRandomObj(nObj))
    waitSecs(1)
    ...
    while len(h_objs) > 0:
        ...
        # h_objs = removeOutOfBoxObjs(arm.clientID, h_objs, xBound2, yBound2)
        h_objs = getInboxHandle(arm.clientID, h_objs, xBound2, yBound2)
